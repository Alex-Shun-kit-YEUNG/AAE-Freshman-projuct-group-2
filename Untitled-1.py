...
class AStarPlanner:
    ...
    def __init__(self, ox, oy, resolution, rr, fc_x, fc_y, tc_x, tc_y):
        ...
        listA=[1000]
        for self.C_F in range(20):
            for self.Delta_F in range(20):
                for self.C_T in range(20):
                    for self.Delta_T in range(20):
                        if self.Delta_F*self.C_T+self.C_F*self.Delta_T>=25 and self.C_F+self.Delta_F>=10 and self.C_T+self.Delta_T>=10:
                            result=self.Delta_F*self.C_T+self.C_F*self.Delta_T+10
                            listA.append(result)
                            if(result==35):
                                print(self.C_F, self.Delta_F, self.C_T, self.Delta_T)
